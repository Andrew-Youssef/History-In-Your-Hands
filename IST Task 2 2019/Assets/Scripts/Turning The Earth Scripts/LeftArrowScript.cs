...
public class LeftArrowScript : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    //This function is called when the left arrow button is held down
    ...
    //This function is called when the left arrow button is let go
    ...
    {
        //If "buttonPressed" is true, then the earth spins to the left at the force declared in "speed"
        if (buttonPressed == true)
        {
            earthObject.transform.Rotate(Vector3.down, speed* Time.deltaTime, Space.World);
        }
    }
}
